...
public abstract class BQ_Service<TService, TEntity>
    ...
    /// </summary>
    /// <param name="ref">
    ///     Object data used as reference, if there's a value will be used otherwise will use sample ones.
    /// </param>
    /// <returns>
    ///     A new sample instance.
    /// </returns>
    /// <remarks>
    ///     Sample instances are no stored data.
    /// </remarks>
    ...
    ///     Sample instances are no stored data.
    /// </remarks>
    protected Driver_Common SampleDriverCommon(bool sampleInternal = true) {
        Driver_Common common = new() {
            License = Entropy[..8],
            Status = SampleStatus("dcm"),
            Situation = SampleSituation(),
            Internal = !sampleInternal ? @ref.Internal : (@ref.Internal ?? SampleDriver()),
            External = !sampleInternal ? @ref.External : (@ref.External ?? SampleDriverExternal()),
        };
