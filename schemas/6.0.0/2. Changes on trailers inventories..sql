...
IF EXISTS (SELECT * FROM sys.triggers WHERE name = 'YardLogs_InsertInto_TrucksInventories')
BEGIN
    DROP TRIGGER YardLogs_InsertInto_TrucksInventories;
    PRINT  'YardLogs_InsertInto_TrucksInventories trigger has been deleted.';
END
GO

IF EXISTS (SELECT * FROM sys.triggers WHERE name = 'TrailerInventories_Management')
BEGIN
    DROP TRIGGER TrailerInventories_Management;
    PRINT  'TrailerInventories_Management trigger has been deleted.';
END
GO
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            BEGIN
				...
				-- Check if the trailer is already stored in the inventory;
				IF EXISTS (
					SELECT 1
					FROM Trailers_Inventories
					WHERE (trailer = @trailerID OR trailerExternal = @trailerExternalID)
				) 
				BEGIN
					DECLARE @trailerInventorySection INT;

					SELECT @trailerInventorySection = Trailers_Inventories.section 
						FROM Trailers_Inventories WHERE (trailer = @trailerID OR trailerExternal = @trailerExternalID)

					DELETE FROM Trailers_Inventories 
					WHERE trailerExternal = @trailerExternalID 
					   OR trailer = @trailerID;
					-- Subtract -1 to section occupancy;
					EXEC Set_SectionOcupancy @sectionID = @trailerInventorySection, @added = 0;
				END
...
BEGIN
    ...
    BEGIN
        BEGIN TRY
            ...
            -- Check if the new trailer is already stored in the inventory;
            IF EXISTS (
                SELECT 1
                FROM Trailers_Inventories
