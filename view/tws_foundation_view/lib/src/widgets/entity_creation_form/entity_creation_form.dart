...
final class EntityCreationForm<T> extends StatefulWidget {
  ...
  /// Entry item designer.
  final Widget Function(T actualModel, bool selected, bool valid) entryDesigner;

  /// Form designer.
  final Widget Function(TWSArticleCreatorItemState<T>? itemState) formDesigner;

  /// A [FutureOr] list for the submit of the added items, returning the result [EntityCreationFormFeedback] status.
  final FutureOr<List<EntityCreationFormFeedback>> Function(List<T> records)? onCreate;

  const EntityCreationForm({super.key, this.controller, this.onCreate, this.validator, this.onClose, required this.factory, required this.entryDesigner, required this.formDesigner});

  @override
  State<EntityCreationForm<T>> createState() => _EntityCreationFormState<T>();
}

class _EntityCreationFormState<TModel> extends State<EntityCreationForm<TModel>> {
  final Router router = Injector.get();

  late _EntityCreationFormState<TModel> mainState;
  ...
  void initState() {
    super.initState();
    mainState = _EntityCreationFormState<TModel>(widget.factory);
  ...
  void submitRecords() async {
    FutureOr<List<EntityCreationFormFeedback>> Function(List<TModel> models)? creator = widget.onCreate;
  ...
  Widget build(BuildContext context) {
    return ReactiveWidget<_EntityCreationFormState<TModel>>(
      reactor: mainState,
      builder: (BuildContext context, _EntityCreationFormState<TModel> state) {
        return LayoutBuilder(
          builder: (_, BoxConstraints cts) {
            final double calcWidth = ((cts.maxWidth / 2) - _kPadding);
            Size sizeFactor = const BoxConstraints(minWidth: _kColWidthLimit).constrain(Size(calcWidth, cts.maxHeight));
            ...
            return Padding(
              ...
              child: Wrap(
                children: <Widget>[
                  SizedBox.fromSize(size: sizeFactor, child: TWSSection(title: 'Properties', content: widget.formDesigner(state.states.isEmpty ? null : state.states[state.current]))),
                  SizedBox.fromSize(
                    size: sizeFactor,
