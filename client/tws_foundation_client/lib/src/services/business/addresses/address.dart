...
/// [country] property key.
  ...
  List<EntityInvalidation<Address>> evaluate() {
    ...
    if(country.length > 3) results.add(EntityInvalidation<Address>(this, PropertyInfo(kCountry, String, country), "Country must be 3 length", "strictLength(3)"));
    ...
    if(street != null && street!.length > 100) results.add(EntityInvalidation<Address>(this, PropertyInfo(kStreet, String, street), "Street must be 100 max length ",  "strictLength(0, 100)"));
    if(altStreet != null && altStreet!.length > 100) results.add(EntityInvalidation<Address>(this,  PropertyInfo(kAltStreet, String, altStreet), "altStreet must be 100 max length ",  "strictLength(0, 100)"));
    if(city != null && city!.length > 30) results.add(EntityInvalidation<Address>(this, PropertyInfo(kCity, String, city), "City must be 30 max length ",  "strictLength(0, 30)"));
    if(zip != null && zip!.length > 5) results.add(EntityInvalidation<Address>(this, PropertyInfo(kZip, String, zip), "ZIP must be 5 length ",  "strictLength(5)"));
    if(subdivision != null && subdivision!.length > 30) results.add(EntityInvalidation<Address>(this, PropertyInfo(kSubdivision, String, subdivision), "Subdivision/Colonia must be 30 max length ",  "strictLength(0, 30)"));
    return results;
  }
