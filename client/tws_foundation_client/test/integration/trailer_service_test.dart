...
void main() {
  ...
  test(
    'Delete',
    () async {
      List<Trailer>  deleteMocks = <Trailer>[];
      int rnd = Random().nextInt(900)  + 99;
      String randomToken = 'D_qual$rnd';
      deleteMocks.add(buildMock(randomToken));
      late Trailer newMock;

      MainResolver<SetBatchOut<Trailer>> fact = await service.create(deleteMocks, auth);
      bool resolved = false;
      fact.resolve(
        decoder: (JObject json) => SetBatchOut<Trailer>.des(json, Trailer.des),
        onException: (Object exception, StackTrace trace) => throw exception,
      ...
      MainResolver<Trailer> deleteFact = await service.delete(
        newMock,
        auth,
      );
      deleteFact.resolve(
