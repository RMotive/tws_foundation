...
final class TWSArticleCreationStackItem extends StatelessWidget {
  ...
  const TWSArticleCreationStackItem({super.key, this.expand = false, this.valid = true, required this.properties, required this.selected});
  ...
  Widget build(BuildContext context) {
    ...
    return DecoratedBox(
      ...
      decoration: BoxDecoration(border: Border.fromBorderSide(BorderSide(width: valid ? 0 : 1.5, color: valid ? Colors.transparent : dangerTheme.accent))),
      child: ColoredBox(
        color: pageTheme.fore.withAlpha(selected ? 126 : 64),
        child: Padding(
          padding: const EdgeInsets.all(8.0),
          child: DefaultTextStyle(
            style: TextStyle(color: pageTheme.accentAlt ?? Colors.black, fontSize: 14, fontWeight: FontWeight.w600),
            child: IntrinsicWidth(child: Wrap(runSpacing: 10, children: properties)),
          ),
        ),
      ),
    );
